...
public final class RequestController {
    ...
    /**
     * This method attaches to the /defaultnodenameid/{objectid} get request
    ...
    public static void getNodeNameWithId(Context context){
        ...
        List<Map> test = Base.findAll("SELECT * FROM default_nodes where id = " + "'"+ idStr+ "'"+ ";"); //raw sql query that returns a map
        ...
        //testNode.saveIt(); //Initalization already saves the object so we don't need to handle this.
        if(nodeStr!=null) {
    ...
    /**
     * This method attaches to the /getdefaultnode/{objectid} get request
     * @param context implicitly passed in context
     */
    public static void getDefaultNode(Context context){
        {
            ...
            try {
                ...
                DefaultNode testNode2 = DefaultNode.findById(nodeId);
     ...
     * These would be structured in the url like : .../getobject/?objectid=foo&objecttable=bar in this methid
     * @param context
     */
    public static void getObjWithIdandTable(Context context) {
        ...
        try {
            String rezStr = gson.toJson(resultSet.get(0).get(objectId));
